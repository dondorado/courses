...
class PhraseTrigger(Trigger):
    ...
    def is_phrase_in(self, text):
        ...
        words_in_phrase = self.phrase.lower().split()
        if words_in_phrase[0] not in words_in_text:
            return False
        else:
            index = words_in_text.count(words_in_phrase[0])
            if index == 1:
                start = words_in_text.index(words_in_phrase[0])
                try:
                    for i in range(start, start + len(words_in_phrase)):
                        if words_in_text[i] != words_in_phrase[i - start]:
                            return False
                    return True
                except IndexError:
                    return False
...
class TimeTrigger(Trigger):
    def __init__(self, date):
        trigger = datetime.strptime(date, "%d %b %Y %H:%M:%S")
        trigger = trigger.replace(tzinfo=pytz.timezone("EST"))
        self.date = trigger

        # self.date = datetime.strptime(date, "%d %b %Y %H:%M:%S").astimezone(pytz.timezone('EST'))
...
class BeforeTrigger(TimeTrigger):
    ...
    def evaluate(self, story):
        ...
        return self.date > pubdate.replace(tzinfo=pytz.timezone("EST"))
