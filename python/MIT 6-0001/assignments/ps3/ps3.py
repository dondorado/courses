...
def get_word_score(word, n):
    ...
    word_score = 0
    for i in word:
        if i.lower() in SCRABBLE_LETTER_VALUES:
            word_score += SCRABBLE_LETTER_VALUES[i.lower()]

    score = word_score * (7 * len(word) - 3 * (n - len(word)))
...
def update_hand(hand, word):
    ...
    hand_copy = hand.copy()
    for i in word:
        if i.lower() in hand_copy and hand_copy[i.lower()] > 0:
            hand_copy[i.lower()] -= 1
            if hand_copy[i.lower()] == 0:
                keys.append(i.lower())
...
def play_hand(hand, word_list):
    ...
    * The hand finishes when there are no more unused letters.
    ...
    hand_copy = hand.copy()
    while True:
        print('Curent hand is:', end=" ")
        for letter in hand_copy:
            for j in range(hand_copy[letter]):
                print(letter, end=" ")
        print()
        ...
        if is_valid_word(user_input, hand_copy, word_list):
            score = get_word_score(user_input, 7)
            total_score += score
            print("{0} earned {1} points. Total: {2} points".format(user_input, score, total_score))
        else:
    ...
    * Returns the total score for the series of hands
    ...
    while True:
        ...
        if input("Would you like to substitute a letter? ") == "yes":
            ...
            hand_copy = substitute_hand(hand_copy, substitute_letter)
